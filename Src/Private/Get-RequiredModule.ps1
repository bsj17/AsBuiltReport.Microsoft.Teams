function Get-RequiredModule {
    ...
    if ($PSVersionTable.PSEdition -eq 'Core') {
        ...
        if ([Version]$ModuleVersion -lt [Version]$Version) {
            throw "Microsoft Teams PowerShell $ModuleVersion is currently installed. Microsoft Teams PowerShell $Version or higher is required to run the Microsoft Teams As Built Report. Please run 'Update-Module -Name $Name -MinimumVersion $Version -Force' to update to the required version."
        }
    }
}
